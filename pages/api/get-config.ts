...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const safeKey = domain.replace(/\./g, '_').replace(/\//g, '__');
    ...
    try {
        ...
        for (const [periodName, days] of Object.entries(periods)) {
            ...
            try {
                ...
                let clickValues: any[] = [];
                try {
                    // Cast to any to avoid "A spread argument must either have a tuple type" build error
                    clickValues = await (kv.mget as any)(...totalClickKeys);
                } catch (e) {
                ...
                try {
                    // Cast to any to avoid "A spread argument must either have a tuple type" build error
                    metrics[periodName].totalUniques = await (kv.pfcount as any)(...totalUniqueKeys);
                } catch (e) {
                ...
                for (const v of variants) {
                    ...
                    try {
                        vClickValues = await (kv.mget as any)(...vClickKeys);
                    } catch (e) {
                    ...
                    let vUniques = 0;
                    try {
                        vUniques = await (kv.pfcount as any)(...vUniqueKeys);
                    } catch (e) {
                        vUniques = 0;
                    }
