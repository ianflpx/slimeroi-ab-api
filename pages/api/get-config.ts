...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        for (const [periodName, days] of Object.entries(periods)) {
            ...
            // CORREÇÃO BUILD: Garantir que o spread seja aceito pelo TS (mget/pfcount esperam rest params ou tuple)
            const clickValues = await (kv.mget as any)(...totalClickKeys);
            ...
            for (const v of variants) {
                ...
                const vClickValues = await (kv.mget as any)(...vClickKeys);
                const vClicks = vClickValues.reduce((acc: number, val: any) => acc + (parseInt(val) || 0), 0);
                const vUniques = await (kv.pfcount as any)(...vUniqueKeys);
