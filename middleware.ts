...
export async function middleware(request: NextRequest, event: import('next/server').NextFetchEvent) {
  ...
  // CORREÇÃO: Suporte para slugs e conversões seguras de chave
  const pathname = request.nextUrl.pathname;
  let lookupDomain = domain;
  
  if (pathname && pathname !== '/' && pathname !== '') {
    const cleanPath = pathname.startsWith('/') ? pathname.substring(1) : pathname;
    lookupDomain = `${domain}/${cleanPath}`;
  }

  let safeKey = lookupDomain.replace(/\./g, '_').replace(/\//g, '__');
  let domainConfig = await get<DomainConfig>(safeKey);

  // Fallback: se não encontrar config para a slug específica, tenta a configuração do domínio base
  if (!domainConfig && lookupDomain !== domain) {
    safeKey = domain.replace(/\./g, '_').replace(/\//g, '__');
    domainConfig = await get<DomainConfig>(safeKey);
  }
  ...
  // Se for uma configuração de slug específico, não repassa o pathname para a URL de destino
  // para evitar erros 404 em checkouts/ofertas.
  let remainingPath = request.nextUrl.pathname;
  if (lookupDomain !== domain) {
    remainingPath = '';
  }
  
  const urlWithParams = new URL(finalTarget + remainingPath + request.nextUrl.search);
