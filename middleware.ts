...
export async function middleware(request: NextRequest) {
  ...
  // --- LOGICA DE METRICAS (REDIS/KV) ---
  try {
    const date = new Date().toISOString().split('T')[0];
    const ip = (request as any).ip || request.headers.get('x-forwarded-for')?.split(',')[0] || '127.0.0.1';

    // Usamos waitUntil para que o rastreamento não atrase a resposta ao usuário
    // Mas no Middleware da Vercel (Edge), basta não esperar (await) se quisermos fire-and-forget,
    // porém para garantir a execução, o ideal é usar o pipeline de forma eficiente.
    ...
    // Executamos sem dar 'await' se quisermos máxima performance, 
    // ou com 'await' para garantir que os dados foram salvos.
    // Na Vercel Edge, 'await' é preferível para evitar cancelamento da promise.
    await pipeline.exec();
  } catch (metricsError) {
