...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // CORREÇÃO: Usando cast para 'any' ou headers diretos para evitar erro de tipo no build da Vercel
    const ip = (request as any).ip || request.headers.get('x-forwarded-for')?.split(',')[0] || '127.0.0.1';

    // Incrementa cliques totais e por variante usando Pipeline para performance
    ...
    // Unicos diários (HyperLogLog é eficiente para contagem de únicos)
    pipeline.pfadd(`metrics:${safeKey}:${variant}:uniques:daily:${date}`, ip);
    pipeline.pfadd(`metrics:${safeKey}:total:uniques:daily:${date}`, ip);

    // Expira as chaves após 90 dias para não crescer infinitamente
    ...
    // Não interrompe o fluxo principal se houver erro no Redis
  }
  // -------------------------------------
